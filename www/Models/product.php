    ...
    Class ProductModel extends Model
    {
        ...
        public function __construct(object $db)
        {
            if(mysqli_query( $db,'select 1 from `ProductTable` LIMIT 1') == FALSE )  {     // If table no exist in db, create one.
               ...
               self::setTestData();                               // The first arg is the name of the Class. It must be exactly the same as the class name.  
            }                                                                            
            $this->db = $db;                                                                                                                                             
        }                                                                                
        ...
        {
            require_once PATH .'/Models/test_data.php';

            foreach ($test_data as $obj)
            {
                $this->createProduct($obj['sku'],$obj['name'],$obj['price'],$obj['productType'],$obj['spec']);
            }
        }
    }
?>                                                     