 <?php   
    ...
    Class Model 
    {
        ...
        {
            $ReflectionClass = new ReflectionClass($this->table_name."Model");                    // Look up the properties in child
            $property_arr = $ReflectionClass->getProperties();                            // Get the properties and the colnum attributes
            $_sql = "CREATE TABLE ".$this->table_name."Table ( ";   

            foreach($property_arr as $key) {
                $name = $key->getName(); 
                                                                // Extract the properties'name
